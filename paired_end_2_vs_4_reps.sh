...
bowtie2 -x $DIRPATH/$INDEX -1 "$CONDITION2"4_R1.fastq -2 "$CONDITION2"4_R2.fastq -S "$CONDITION2"_SAMPLES4.sam
...
htseq-count -s $STRANDED -t $FEATURE -a 1 -m intersection-nonempty -i ID "$CONDITION2"_SAMPLES4.sam "$INDEX".gff > secondary_"$CONDITION2"_counts4.txt
